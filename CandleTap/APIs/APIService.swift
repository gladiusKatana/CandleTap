...
//                    {candleSubset = krakenBtcPrices}
            ...
            self?.getBinanceFeed { responses in
                binanceLatestPrice = responses
                let bncPrice = "\(binanceLatestPrice!.last!.price!)"                //; print("latest binance price: \(bncPrice)")
                latestBinanceETHBTC = Double("\(bncPrice)")!
        ...
        if !fetchHistoricalDataAutomatically && chartDisplayed {processCandleSubset()}
    }
